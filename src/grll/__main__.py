...
def main(args):
    ...
    if args.label_file and args.method != 'gcn':
        ...
        results = clf.split_train_evaluate(X, Y, args.clf_ratio, seed=0)

        accuracy = open('accuracy.txt', 'a')

        accuracy.write("Dataset:{}, model:{}, ratio:{}, dim:{}\n".\
            format(dataset, args.method, args.clf_ratio, args.representation_size))
        accuracy.write("accuracy:{}\n\n".format(results['micro']))
